...
import { createCanvasAutosaveScheduler } from "../src/hooks/canvas-autosave-scheduler.ts";
...
test("does not invoke save or status callbacks after dispose", async () => {
  const savedSnapshots: string[] = [];
  const statuses: string[] = [];
  const scheduler = createCanvasAutosaveScheduler({
    delayMs: 10,
